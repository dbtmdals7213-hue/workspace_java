...
class BadLoginServlet {
	...
	public String service(String userId) {
		...
		//1단계:	요청받은 아이디를 멤버 변수에 저장한다.
		loginId = userId;
		
		// 로그인 처리 시간을 흉내낸다.(DB 조회 등으로 5 밀리초 걸린다고 가정)
		// - 이 사이에 다른 스레드가 loginId 를 덮어 쓸 수 있다.
		try {
		...
		//2단계:	멤버 변수를 읽어서 브라우저로 응답을 만든다.
		return loginId + "님 환영합니다.";
	}// === service Method
	
}// --- BadLoginServlet Class
...
class GoodLoginServlet {
	...
	public String service(String userId) {
		...
		// 매개변수 userId 는 지역변수 -> 스레드마다 자기 것을 따로 가진다.
...
class LoginRequest implements Runnable {
	...
	public void run() {
		...
		// 잘못된 서블릿에 로그인 요청 -> 멤버 변수 loginId 의 값이 섞일 수 있다.
		String badResult = badServlet.service(userId);
		
		// 올바른 서블릿에 로그인 요청 -> 지역변수 loginId 의 값은 정확한 값을 사용할 수 있다.
...
public class Thread06_ServletSim {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// 서블릿은 각각 1개 만든다.
		BadLoginServlet bad = new BadLoginServlet(); // 잘못된 서블릿 <- 멤버 변수 loginId 존재
		...
		// 로그인 세 요청이 모두 끝날 때까지 main 스레드 기다리게 하기
		t1.join();
		t2.join();
		t3.join();
