				   ...
				   객체 메모리 내부의  APPLE_PRICE상수의 final선언이 사라진점이 문제가됨
				    ...
				    final선언이 사리진점이 왜 문제가 될까?
				     : 클래스 설계시 과일장수가 판매하는 사과 한개당 가격은 변경되지 않음을 가정해 두고
				       ...
				       개발자가 객체 메모리의 변수 값 초기화 하는작업의 실수를 범할수도 있기때문에
				       객체 메모리 생성하는 동시에 단 한번만 호출되는 생성자를 이용해
				       객체 메모리를 생성후 빠르게 객체변수들을 초기화 해야 합니다.
				
		이 프로그램의 문제점들 해결방법은  클래스설계시 내부에 생성자를 만들어 놓고 사용하자
	*/
