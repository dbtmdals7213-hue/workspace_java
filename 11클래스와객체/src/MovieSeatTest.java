...
// - 데이터: 좌석 번호(seatNumber), 예약 여부(isReserved), 예약자(reservedBy)
...
public class MovieSeatTest {
	...
	//클래스 메소드 선언
		/*
			메소드명: reserve
			기능:	 예약이 안 되어 있으면 customer 로 예약 처리,
		...
		*/
		void reserve(String customer) {
			...
			if(!isReserved) {// 아직 예약되지 않은 좌석인지 검사
				...
				isReserved = true;
				reservedBy = customer;
				System.out.println(seatNumber + " 좌석, " + reservedBy + "님 예약 완료"); // 예약 완료 메세지 출력
			}else {// 이미 예약된 좌석인지 검사
		...
		*/
		...
		/*
			메소드명: printSeat
			기능: 좌석 번호, 예약 여부, 예약자를 한 줄로 출력한다
		*/
		void printSeat() {
			
			System.out.println("좌석 번호: " + seatNumber + " / 예약 여부: " + isReserved + " / 예약자: " + reservedBy);
		}
