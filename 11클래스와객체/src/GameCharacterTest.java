...
// 클래스 메소드 선언
	/*
		메소드명: takeDamage
		기능:	 hp 를 dmg 만큼 줄이고, 0 이하가 되면 0으로 고정 후
			 사망 메시지 출력, 아니면 남은 체력 출력
	*/	
	void takeDamage(int dmg) {
		
		hp -= dmg; // 현재 체력에서 받은 피해량 만큼 감소
		
		if(hp <= 0) {// 체력이 0이하로 떨어졌는지 검사
			
			hp = 0; // 음수가 되지 않도록 0으로 고정
			System.out.println(name + " 사망했습니다."); // 사망 메시지 출력
		}else {// 체력이 남아있지 않는 경우 검사
	...
	/*
		메소드명: gainExp
		기능:	 exp 에 amount 를 더하고, 100 이상이면 레벨업(exp 초기화),
			 아니면 경험치 획득 메시지 출력
	*/
