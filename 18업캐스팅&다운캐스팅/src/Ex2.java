...
public class Ex2 {
	...
	public static void main(String[] args) {
		...
		// 업 캐스팅 기능을 사용하지 않은 예
		// 단점:	c, c1, c2 참조변수를 여러 개 만들어 사용해야 합니다.
		...
		// 업 캐스팅 기능을 사용한 예
		// 장점:	부모 클래스 자료형의 참조변수 하나만 만들어 여러 자식 객체 메모리를 생성해서 저장 가능
		
		// 업 캐스팅 작성 방법
		// 부모클래스자료형 참조변수 = new 자식클래스의생성자();
		Parent p = new Child();
		p.parentPrn();
		// p.childPrn(); -> 자식 Child 클래스에 만들어 놨던 메소드 사용 불가
		
		// 업 캐스팅의 단점: 부모 클래스 Parent 에 만들어 놓은 parentPrn() 메소드만 호출 가능하고 자식 Child 클래스에 만들어 놓은 메소드 사용 불가
		
		// 업 캐스팅의 단점을 보완하기 위해 다운 캐스팅 기능을 사용하자
	}
