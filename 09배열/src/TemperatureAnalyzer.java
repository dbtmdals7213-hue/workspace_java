...
public class TemperatureAnalyzer {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < temps.length; i++) {
			...
			// 5. (for 문 블록 안) 만약 현재 꺼내온 기온이 minTemp보다 작다면?
			// minTemp의 값을 현재 기온으로 변경(갱신)하는 if 문을 작성하세요.
			if(temps[i] < minTemp) {
				
				minTemp = temps[i]; // 기존 값보다 더 작은 기온을 발견하면 최소값 교체!
			}
			
			// 6. (for 문 블록 안) 만약 현재 꺼내온 기온이 0보다 작다면(영하권이라면)?
			// subZeroCount를 1 증가시키는 if 문을 작성하세요.
			if(temps[i] < 0) {
		...
		// 7. (for 문이 완전히 끝난 후) 아래 양식에 맞게 결과를 출력하세요.
		// 출력 예시:
		// 가장 추웠던 날의 기온: -5도
		// 영하권이었던 날의 수: 2일
