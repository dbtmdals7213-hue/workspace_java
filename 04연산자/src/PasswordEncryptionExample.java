...
참고1.
	...
	위와 같이 password 변수 메모리를 String 클래스 자료형으로 선언 후 문자열을 저장 해 놓았을때.
	
	문자열 "myPassword123" 을 저장한 String 객체 메모리가 만들어집니다.
	
	이때  String 클래스에 이미 만들어서 제공하는 메소드 중에서 charAt(index) 메소드는 하나의 기능을 가지고 있는데
...
// 주제: 왼쪽 / 오른쪽 shift 연산자를 사용하여 비밀번호를 암호화하고 복호화하는 간단한 예제
...
public class PasswordEncryptionExample {
	...
	public static void main(String[] args) {
		...
		int shift = 3;	// shift 변수는 비트 시프트 연산에 사용될 값을 정합니다.
						// 여기에서는 3을 선택하여 왼쪽으로 3개의 비트를 시프트 할 것입니다.
		...
		for(int i = 0; i < password.length(); i++) {
			...
			// 각 문자의 ASCII 코드 값의 2진수 값을 왼쪽으로 시프트하여 암호화
			/*
				예
	        	...
	        	3비트 shift 하면 오른쪽에 추가되는 비트는 버려지고, 0으로 채워집니다. 
	        	
	        	따라서 01101101을 왼쪽으로 3비트 shift 한 결과는 01101000가 됩니다.
	        	
	        	결과적으로 "myPassword123".charAt(0) << 3는 'm' 의 ASCII 코드인 109를 왼쪽으로 3비트 shift 한 것이므로, 
	        	
	        	이진수로 나타내면 01101000입니다.
	        	
	        	정수로 표현하면 104가 됩니다. 따라서 결과는 104입니다.
	        	
	        	(char)(104); 104 10진수를 문자로 강제 형 변환 하면 'h' 문자로 만들어 줍니다.
			*/
		...
		//암호화된 비밀번호: ψʀ̈ΘΘθ͸ΐ̠ƈƐƘ  
        /*
			🎯 왜 ψʀ̈ΘΘθ͸ΐ̠ƈƐƘ 같은 이상한 문자가 나올까?
		...
		*/
		/*
			암호화 된 비밀번호가 이상하게 출력되는 이유는 해당 코드에서 사용한 비트 시프트 연산이 잘못되었기 때문입니다. 
	        비트 시프트 연산은 산술이나 논리적 연산을 수행할 때 사용하는 연산자인데, 비트를 왼쪽이나 오른쪽으로 이동시키는 연산입니다. 
	        그러나 이 코드에서는 이 비트 시프트 연산을 이용하여 문자열을 암호화하고 복호화하고 있습니다. 
	        이 코드에서 사용한 shift 값을 3으로 설정하면, 비트 연산이 원하지 않는 방향으로 되어 원래의 문자열과 다른 형태로 암호화가 진행되는 것입니다.
	        즉, 암호화 된 문자열 출력이 이상하게 되는 것입니다. 
		...
		*/
		// decryptedPassword 변수를 선언하여 복호화 한 비밀번호를 저장할 것입니다.
