/*
	...
	암호화는 XOR 연산을 사용하며, 비밀 키로는 숫자를 사용합니다.
	암호화하려는 문자열의 각 문자를 비밀 키로 XOR 연산을 수행하여 암호화 한 문자열을 만듭니다. 
	복호화는 암호화 된 문자열의 각 문자를 다시 비밀 키로 XOR 연산을 수행하여 원래의 문자열을 복원합니다.
...
public class BitwiseEncryptionExample {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < password.length(); i++) {
            // 각 문자의 아스키 코드 값의 2진수 값을 비밀 키 int key = 42; 와 XOR 연산하여 암호화
        	// 예
        	/*
        	(char)('m' ^ 42)는 Java 에서 비트 XOR 연산을 수행하고 그 결과를 문자로 변환하는 식입니다.
        ...
        System.out.println("복호화 된 비밀번호: " + decryptedPassword);
    }
