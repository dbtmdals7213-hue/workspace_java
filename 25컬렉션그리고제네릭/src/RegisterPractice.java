/*
	...
	■ 문제 0 (서술형) : 아래 두 질문에 주석으로 답하시오.
	   (1) 중복 신청을 걸러내는 용도로 HashSet을 쓰는 이유는?
	       답: HashSet 은 이미 저장된 것과 내용이 같은 객체의 add 를 거부하고
			  false 를 반환하므로, add 의 반환 값만 확인하면 별도의 검색 없이
			  "처음 신청인지 / 중복 신청인지" 를 바로 판별할 수 있기 때문이다.
			  
	   (2) 합격자/대기자 명단으로 ArrayList를 쓰는 이유는?
	       답: 명단은 "신청 순서" 가 유지되어야 하는데, HashSet 은 저장 순서를 보장하지 않고,
	       	  ArrayList 는 add 한 순서 그대로 저장되기 때문이다.
