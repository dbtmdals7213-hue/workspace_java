...
// Field 클래스
// - 리플렉션을 통해 특정 클래스의 변수 정보를 다루기 위한 클래스
...
public class ReflectionPrivaateFieldExample {

	public static void main(String[] args) throws Exception {
		
		//1. 리플렉션 기법으로 정보를 조사할 Member 클래스의 객체 생성
		Member member = new Member();
		// new Member(); <- 기본 생성자를 호출해 객체 생성하면 String name = null, int age = 0; 으로 저장될 것임
		...
		//3. Member 클레스에 만들어 놓은 변수 정보 얻기
		Field nameField =claszz.getDeclaredField("name");
		// Class 라는 객체 통해서 "name" 라는 이름을 가진 변수 정보 검색
		// private 로 선언된 name 변수도 포함하여 검색됩니다.
		// 만약 name 변수를 찾지 못하면 NoSuchFieldException 예외가 발생됩니다.
		
		//4. private 접근 제한 해제
		nameField.setAccessible(true);
		// JVM 에게 접근 제어 검사를 하지 않도록 요청
		// private -> 접근 가능 상태로 변경
		// 없으면 IllegalAccessException
		...
		// member 객체의 age 인스턴스 변수에 정수 값 30을 강제로 저장
...
}// --- ReflectionPrivaateFieldExample 클래스
