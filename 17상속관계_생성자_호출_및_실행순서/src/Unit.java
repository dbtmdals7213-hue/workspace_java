...
public class Unit {
	...
	public Unit(String name) {
		...
		super();
		this.name = name;
		this.hp = 100; // 기본 체력 100
		this.damage = 10; // 기본 공격력 10
	}
	
	// 메소드들
	//1. 상대 유닛 Marine 객체를 공격하는 행동의 메소드
	public void attackMarin(Marin target) {// 매개변수로 new Marin(); 객체 주소 전달 받는다.
		...
		// 저글링이 마린을 공격!
		// Marin 상대 유닛의 체력을 차감하기 위해 공격하는 유닛 객체의 공격력만큼 감소시키자
	...
	//2. 상대 유닛 Zergling 객체를 공격하는 행동의 메소드
	public void attackZergling(Zergling target) {// 매개변수로 new Zergling("저글링", 80); 객체 주소 전달 받는다.
		...
		// Tank 가 Zergling 을 공격!
		// Zergling 상대 유닛의 체력을 차감하기 위해 공격하는 유닛 객체의 공격력만큼 감소시키자
		target.hp -= this.damage;
		System.out.println(this.name + "이(가) " + target.name + "을(를) 공격합니다! (공격력: " + this.damage + ")");
		
		// Zergling 상대 유닛의 체력이 0이 되면
		if(target.hp <= 0) {
	...
	public void attackTank(Tank target) {// 매개변수로 new Tank("시저탱크", 150, 35); 객체 주소 전달 받는다.
		...
		if(target.hp <= 0) {// new Tank("시저탱크", 150, 10).hp <= 0
			
			System.out.println(target.name + "은(는) 이미 파괴되어 공격할 수 없습니다.");
		}
		
		// Zergling 이 Tank 를 공격!
		// Tank 상대 유닛의 체력을 차감하기 위해 공격하는 유닛 객체의 공격력만큼 감소시키자
		target.hp -= this.damage;
		System.out.println(this.name + "이(가) " + target.name + "을(를) 공격합니다! (공격력: " + this.damage + ")");
		
		// Tank 상대 유닛의 체력이 0이 되면
		if(target.hp <= 0) {
	...
	//4. 현재 유닛 상태(체력 그리고 공격력)를 출력하는 기능의 메소드
	public void status() {
